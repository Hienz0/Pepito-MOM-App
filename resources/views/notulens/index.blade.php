...
<nav style="background-color: #F9F9F9;" class="fixed w-full shadow-md h-20 top-0 left-0">
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
        <div class="flex items-center justify-between h-full">
            ...
            <div class="flex items-center space-x-4">
                ...
                <!-- User Icon -->
                <div class="relative">
                    <button type="button" id="userMenuButton" class="flex items-center text-gray-700 hover:text-gray-900 focus:outline-none">
                        <i class="fa-regular fa-user text-4xl"></i>
                    </button>
                    <!-- Dropdown Menu -->
                    <div id="userDropdown" class="hidden absolute right-0 mt-2 w-60 bg-white rounded-md shadow-lg z-[9999]">
                        ...
                        <div class="px-4 py-2">
                            <form action="{{ route('logout') }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full text-left text-gray-700 hover:text-gray-900">Logout</button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <div class="container mx-auto mt-10">
        ...
        <div class="flex justify-between mb-6">
            ...
            <i class="fas fa-circle-plus mr-2"></i> Add New MoM
         </a>
           ...
           <!-- Search Bar -->
    <div class="mb-6">
        <input type="text" id="searchInput" placeholder="Search by title or ID" class="px-4 py-2 border rounded w-full">
    </div>
        ...
        <div class="shadow-lg bg-white rounded-lg overflow-hidden z-[1]">
            <div class="p-6">
                    ...
                    <tbody id="notulenTable" class="divide-y divide-gray-200">
                        ...
                        <tr onclick="window.location='{{ route('notulens.show', $notulen->id) }}'" class="hover:bg-gray-100 cursor-pointer">
                            ...
                            <td class="p-4 flex space-x-2">
                                <!-- View Icon -->
                                <a href="{{ route('notulens.show', $notulen->id) }}" class="text-blue-500 hover:text-blue-700" data-bs-toggle="tooltip" title="View">
                                ...
                                <!-- Edit Icon -->
                                <a href="{{ route('notulens.edit', $notulen->id) }}" class="text-yellow-500 hover:text-yellow-700" data-bs-toggle="tooltip" title="Edit">
                                ...
                                <form action="{{-- route('notulens.inactivate', $notulen->id) --}}" method="POST" class="inline-block">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="text-red-500 hover:text-red-700" data-bs-toggle="tooltip" title="Inactivate" onclick="return confirm('Are you sure you want to inactivate this notulen?');">
                ...
                <div class="flex justify-between items-center mt-6">
                    <button id="prevPage" class="bg-[#79B51F] hover:bg-[#69A01C] text-white font-bold py-2 px-4 rounded flex items-center">
                        <i class="fas fa-arrow-left mr-2"></i>Previous
                    </button>
                    
                    <div id="pagination" class="flex space-x-2"></div>

                    <button id="nextPage" class="bg-[#79B51F] hover:bg-[#69A01C] text-white font-bold py-2 px-4 rounded flex items-center">
    ...
    document.addEventListener('DOMContentLoaded', function () {
     ...
     document.addEventListener('DOMContentLoaded', function() {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            const id = rows[i].cells[0].dataset.id; // Assume ID is stored in a data attribute or can be retrieved
